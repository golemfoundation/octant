...
def test_antisybil_service(
    ...
    score, stamps = service.fetch_antisybil_status(context, alice.address)
    assert score.score == 2.572
    assert len(stamps) == 3
    assert score.expires_at == datetime.strptime(
        "2090-01-01T00:00:00Z", "%Y-%m-%dT%H:%M:%SZ"
    )

    service.update_antisybil_status(
        context, alice.address, score.score, score.expires_at, stamps
    )

    score = service.get_antisybil_status(context, alice.address)
    assert score.score == 2.572
...
def test_guest_stamp_score_bump_for_both_gp_and_octant_side_application(
    ...
    score = service.get_antisybil_status(context, alice.address)
    assert score.score == 2.572  # guest list score bonus not applied

    guest_address = "0x2f51E78ff8aeC6A941C4CEeeb26B4A1f03737c50"
    database.user.add_user(guest_address)
    score, stamps = service.fetch_antisybil_status(context, guest_address)
    service.update_antisybil_status(
        context, guest_address, score.score, score.expires_at, stamps
    )
    score = service.get_antisybil_status(context, guest_address)
    ...
    score, stamps = service.fetch_antisybil_status(context, stamp_address)
    service.update_antisybil_status(
        context, stamp_address, score.score, score.expires_at, stamps
    )
    score = service.get_antisybil_status(context, stamp_address)
    assert (stamps) and (
        score.score == 22.0
