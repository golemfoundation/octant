...
def test_user_nonce_for_non_existent_user_is_0(service, alice):
    ...
    assert service.get_next_user_nonce(alice.address) == 0
...
def test_user_nonce_changes_increases_at_each_allocation_request(
    ...
    new_nonce = service.get_next_user_nonce(alice.address)
...
def test_user_nonce_changes_increases_at_each_allocation_request_for_each_user(
    ...
    for i in range(0, 5):
        ...
        next_user_nonce = service.get_next_user_nonce(alice.address)
        assert next_user_nonce == i + 1

        # for other users, nonces do not change
        assert service.get_next_user_nonce(bob.address) == 0
        assert service.get_next_user_nonce(carol.address) == 0

    for i in range(0, 4):
        database.allocations.store_allocation_request(bob.address, 0, _mock_request(i))
        next_user_nonce = service.get_next_user_nonce(bob.address)
        assert next_user_nonce == i + 1

        # for other users, nonces do not change
        assert service.get_next_user_nonce(alice.address) == 5
        assert service.get_next_user_nonce(carol.address) == 0
    ...
    for i in range(0, 3):
        ...
        next_user_nonce = service.get_next_user_nonce(carol.address)
        assert next_user_nonce == i + 1

        # for other users, nonces do not change
        assert service.get_next_user_nonce(alice.address) == 5
        assert service.get_next_user_nonce(bob.address) == 4
