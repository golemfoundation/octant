...
@pytest.fixture
def calculator():
    return UserBudgetWithPPF()


@pytest.fixture
def payload():
    return UserBudgetPayload(
        user_effective_deposit=100,
        total_effective_deposit=200,
        all_individual_rewards=50,
        ppf=100,
    )
...
@pytest.mark.parametrize(
    ...
    [
        (1, 100, 90, 350, 1_750000000_000000000),
        (1, 100, 150, 350, 1_750000000_000000000),
...
def test_ppf_user_budget(calculator):
    payload = UserBudgetPayload(USER1_ED, TOTAL_ED, ALL_INDIVIDUAL_REWARDS, PPF)
...
def test_ppf_user_budget_total_effective_equals_0(calculator):
    payload = UserBudgetPayload(USER1_ED, 0, ALL_INDIVIDUAL_REWARDS, PPF)
    result = calculator.calculate_budget(payload)

    assert result == 0


def test_ppf_user_budget_total_effective_is_none(calculator):
    payload = UserBudgetPayload(
