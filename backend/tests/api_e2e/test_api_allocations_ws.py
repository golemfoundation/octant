...
def test_allocations_via_socketio(
    ...
    print(f"Testing socketio with {len(alice_proposals)} proposals and amount {allocation_amount}")
    ...
    async def run_test():
        ...
        async def on_project_rewards(data):
            print(f"Received project_rewards event")
        ...
        try:
            ...
            if not results.connected:
                ...
                while (results.threshold is None or 
                        results.project_rewards is None or 
                        len(results.project_donors) < len(alice_proposals)) and asyncio.get_event_loop().time() - start_time < timeout:
                    
                    # Log status every second
                    current_time = asyncio.get_event_loop().time()
                    if current_time - last_log_time >= 1.0:
                        print(f"Waiting for events... Time elapsed: {current_time - start_time:.1f}s")
                        ...
                        print(f"  Project donors received: {len(results.project_donors)}/{len(alice_proposals)}")
                ...
                print(f"Received {len(results.received_events)} events: {results.received_events}")
                print(f"Threshold received: {results.threshold is not None}")
                print(f"Project rewards received: {results.project_rewards is not None}")
                print(f"Project donors received: {len(results.project_donors)}/{len(alice_proposals)}")
    ...
    user_allocations, status_code = fclient.get_user_allocations(STARTING_EPOCH + 1, ua_alice.address)
    assert status_code == 200, f"Expected status code 200, got {status_code} when getting user allocations"
    ...
    for proposal in alice_proposals:
        ...
        for allocation in user_allocations["allocations"]:
            if allocation["address"] == proposal:
                assert int(allocation["amount"]) == allocation_amount, f"Expected amount {allocation_amount}, got {allocation['amount']}"
                found = True
                break
