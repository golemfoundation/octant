...
def test_computes_weighted_deposit_for_two_events():
    ...
    assert result == [WeightedDeposit(100, 50), WeightedDeposit(220, 150)]
...
def test_computes_user_deposits_when_deposited_during_epoch():
    ...
    assert result == [WeightedDeposit(0, 17), WeightedDeposit(100, EPOCH_DURATION - 17)]
...
def test_computes_user_deposits_when_unlocked():
    deposits = [(EPOCH_START, 100), (EPOCH_START + 150, -100)]
    payload = create_payload(deposits)
    uut = DefaultTimebasedWeights()
