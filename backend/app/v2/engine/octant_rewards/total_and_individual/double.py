...
class DoubleTotalAndIndividualRewards:
    ...
    def calculate_total_rewards(self, payload: TotalAndAllIndividualPayload) -> int:
        if payload.locked_ratio < self.DOUBLING_GLM_SUPPLY_LIMIT:
            return (
                int(payload.eth_proceeds * payload.locked_ratio.sqrt()) * self.REWARDS_MULTIPLY_FACTOR
            )
        else:
    ...
    def calculate_all_individual_rewards(self, payload: TotalAndAllIndividualPayload) -> int:
        if payload.locked_ratio < self.DOUBLING_GLM_SUPPLY_LIMIT:
            return int(payload.eth_proceeds * payload.locked_ratio) * self.REWARDS_MULTIPLY_FACTOR
        else:
