    ...
    const parameters: TestParam[] = [
      ...
      },
      {
        steps: [{
          Alice: 1000,
          forwardEpochs: 5
        }, { Alice: -100 }, { Alice: -100 }, { Alice: -100 }, { forwardEpochs: 5 }],
        tests: [{ epoch: 2, expDAlice: 1000 }, { epoch: 5, expDAlice: 1000 }, {
          epoch: 6,
          expDAlice: 700
        }],
      ...
      {
        steps: [{ Alice: 50, forwardEpochs: 1 }, { Alice: 100, forwardEpochs: 2 }, {
          Alice: -99,
          forwardEpochs: 2
        }],
        tests: [{ epoch: 1, expDAlice: 0 }, { epoch: 3, expDAlice: 150 }, {
          epoch: 4,
          expDAlice: 0
        }],
      ...
      {
        steps: [{ Alice: 60, forwardEpochs: 1 }, { Alice: 60, forwardEpochs: 2 }, {
          Alice: -100,
          forwardEpochs: 2
        }],
        tests: [{ epoch: 2, expDAlice: 0 }, { epoch: 3, expDAlice: 120 }, {
          epoch: 5,
          expDTotal: 0
        }],
      ...
      },
      {
        ...
        tests: [{ epoch: 2, expDTotal: 0 }, { epoch: 3, expDTotal: 120 }, {
          epoch: 5,
          expDTotal: 0
        }],
  ...
  describe('Effective deposits, edge cases', async () => {
    ...
    it('tracker accepts calls only from deposits', async () => {
      ...
      expect(tracker.connect(signers.Darth).processDeposit(signers.Darth.address, 0, parseEther('100000'))).to.be.revertedWith(
        'HN:Tracker/unauthorized-caller'
      );
