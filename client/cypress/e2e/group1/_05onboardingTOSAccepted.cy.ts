...
const { isMobile, isTablet, isDesktop, isLargeDesktop } = getViewports();

describe(`[AW IS CLOSED] onboarding (TOS accepted):`, () => {
  before(() => {
    beforeSetup();
  });
  ...
  it('Onboarding stepper has right amount of steps and highlights correct amount of passed steps', () => {
    ...
    cy.get(`[data-test*=OnboardingStepper__circle]`).should('have.length', onboardingSteps.length);
