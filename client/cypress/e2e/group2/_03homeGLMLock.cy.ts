...
const { isDesktop } = getViewports();

chai.use(chaiColors);

describe(`[AW IS CLOSED] HomeGlmLock:`, () => {
  let lockedGlms = 0;
  ...
  it('Effective value is 0 (GLM/$) when wallet isn`t connected ', { scrollBehavior: false }, () => {
  ...
  it(
    ...
    () => {
      ...
      cy.get('[data-test=HomeGridCurrentGlmLock__Button]').invoke('text').should('eq', 'Lock GLM');
    },
  );
  ...
  it('Wallet connected: Lock 1000 GLM', { scrollBehavior: false }, () => {
    ...
    cy.get('[data-test=HomeGridCurrentGlmLock--current__primary]')
      ...
      .then(text => {
        ...
        cy.get('[data-test=LockGlmTabs__Button]', { timeout: 180000 }).should('have.text', 'Close');
        ...
        cy.get('[data-test=TransactionsListItem__title]').first().should('have.text', 'Locked GLM');
  ...
  it('Wallet connected: Unlock 1 GLM', { scrollBehavior: false }, () => {
    ...
    cy.get('[data-test=HomeGridCurrentGlmLock--current__primary]')
      ...
      .then(text => {
        ...
        cy.get('[data-test=LockGlmTabs__Button]', { timeout: 60000 }).should('have.text', 'Close');
        cy.get('[data-test=LockGlmNotification--success]').should('be.visible');
        cy.get('[data-test=LockGlmTabs__Button]').click();
        cy.wait(5000);
        cy.get('[data-test=HomeGridCurrentGlmLock]').scrollIntoView({
