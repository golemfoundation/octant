...
import viewports, { getViewports } from 'cypress/utils/viewports';
...
const { isDesktop, isLargeDesktop } = getViewports();

describe(`[AW IS OPEN] Settings:`, () => {
  before(() => {
    beforeSetup();
  });
  ...
  beforeEach(() => {
    ...
    visitWithLoader(
      ...
      isDesktop ? ROOT_ROUTES.home.absolute : ROOT_ROUTES.settings.absolute,
    );
  });
  ...
  it('Settings title is visible and has correct text', () => {
    const dataTestRoot = isDesktop ? 'Settings' : 'SettingsView';
